...
app.post('/login', userController.verifyUser, cookieController.setSSIDCookie, sessionController.startSession, (req, res) => {
  ...
  if (res.locals.user === null) res.redirect('/signup')
...
app.get('/secret', sessionController.isLoggedIn, (req, res) => {
  if (res.locals.session === null) res.redirect('/signup')
  else res.sendFile(path.resolve(__dirname, '../client/secret.html'));
});

app.get('/secret/users', sessionController.isLoggedIn, userController.getAllUsers, (req, res) => {
  if (!req.cookies.ssid) res.redirect('/')
  else res.send( { users: res.locals.users });
})
