...
const PackChart = ({ data, options, hoveredMicroservice }) => {
  ...
  useEffect(() => {
    ...
    function handleZoom(e) {
      ...
      d3.selectAll('text')
        .style('visibility', e.transform.k > 1 ? 'visible' : 'hidden');
    }
...
const Pack = (data, options) => {
  ...
  const filterLinks = (links, hoveredNode) => {
    return links.filter(link => link.source === hoveredNode || link.target === hoveredNode);
  };
  ...
  const simulation = d3.forceSimulation()
    .force("collide", d3.forceCollide().strength(.1).radius(2))
    .alphaDecay(0)
    .alpha(1);
