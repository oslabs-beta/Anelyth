...
console.log ('In extract details middleware!!!')
    ...
    function traverseHierarchy(node) {
        if (node.type === 'file' && node.info) {
            ...
            if (node.info.dependencies) {
                const validDependencies = node.info.dependencies.filter(dep => !dep.couldNotResolve && dep.resolved.includes('/'));
    ...
    try {
        ...
        detailsArray = detailsArray.filter(file => !(file.fileName.endsWith('.json') || file.fileName.endsWith('.md')));

        // Write the output to a file
        fs.writeFileSync('structure_for_ross.log', JSON.stringify(detailsArray, null, 2));
    } catch (error) {
